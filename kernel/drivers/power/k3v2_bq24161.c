...
struct k3_bq24161_device_info {
	...
	unsigned  int 		limit_voltagemV;
    unsigned  int       limit_currentmA;
...
enum{
	...
	BATTERY_HEALTH_TEMPERATURE_HIGH_OVERHIGH,
};
 ...
 */
static int bq24161_get_boardid_charge_parameter(struct k3_bq24161_device_info *di)
{
    bool ret = 0;
    ...
    case BAT_ID_10K_STATUS:
		 ...
		 ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_10k", &di->limit_voltagemV, NULL);		 
         ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_10k", &di->limit_currentmA, NULL);		 
         break;
    case BAT_ID_22K_STATUS:
		 ...
		 ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_22k", &di->limit_voltagemV, NULL);		 
         ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_22k", &di->limit_currentmA, NULL);		 
         break;
    case BAT_ID_39K_STATUS:
		 ...
		 ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_39k", &di->limit_voltagemV, NULL);		 
         ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_39k", &di->limit_currentmA, NULL);		 
         break;
    case BAT_ID_68K_STATUS:
		 ...
		 ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_68k", &di->limit_voltagemV, NULL);		 
         ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_68k", &di->limit_currentmA, NULL);		          
         break;
    case BAT_ID_110K_STATUS:
		 ...
		 ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_110k", &di->limit_voltagemV, NULL);		 
         ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_110k", &di->limit_currentmA, NULL);		 
         break;
    case BAT_ID_200K_STATUS:
		 ...
		 ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_200k", &di->limit_voltagemV, NULL);		 
         ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_200k", &di->limit_currentmA, NULL);		 
         break;
    case BAT_ID_470K_STATUS:
		 ...
		 ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_470k", &di->limit_voltagemV, NULL);		 
         ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_470k", &di->limit_currentmA, NULL);		 
         break;
    case BAT_ID_2M_STATUS:
		 ...
		 ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_2m", &di->limit_voltagemV, NULL);		 
         ret&= get_hw_config_int("gas_gauge/limit_charge_voltage_2m", &di->limit_currentmA, NULL);		 
         break;
    default:
		 ...
		 ret&= get_hw_config_int("gas_gauge/limit_charge_voltage", &di->limit_voltagemV, NULL);	
         ret&= get_hw_config_int("gas_gauge/limit_charge_voltage", &di->limit_currentmA, NULL);		 	 
         break;
    }
    if(ret){
        return true;
		...
		di->limit_voltagemV = BQ24161_TEMPRATURE_HIGH_LIMIT_VOLTAGE;
        di->limit_currentmA = BQ24161_TEMPRATURE_LOW_LIMIT_CURRENT;
...
static int bq24161_get_max_charge_voltage(struct k3_bq24161_device_info *di)
{
    bool ret = 0;
    int battery_id_status = 0;
...
static int k3_bq24161_check_battery_temperature_threshold(void)
{
	...
	/* UP:
	 * temperature < 0 		 : stop charge
     * 0 <= temprature <= 10 : less than 0.3C charge
	 * 10 < temprature <= 42 : norma charge
	 * 42 < temprature <= 52 : limit charge
	 * 52 < temprature       : stop charge
	 * DOWN:
	 * 50 < temprature       : stop charge
	 * 42 < temprature <= 50 : limit charge
	 * 10 < temprature <= 42 : normal charge 
     * 0 <= temprature <= 10 : less than 0.3C charge
	 */
	  ...
	  && (battery_temperature <= BQ24161_WARM_BATTERY_THRESHOLD) )
	{
	  ...
	  && (battery_temperature <= (BQ24161_HOT_BATTERY_THRESHOLD-TEMPERATURE_OFFSET)))
	{
        return BATTERY_HEALTH_TEMPERATURE_HIGH;

	} 		
	else if ((battery_temperature > (BQ24161_HOT_BATTERY_THRESHOLD-TEMPERATURE_OFFSET))
	  && (battery_temperature <= BQ24161_HOT_BATTERY_THRESHOLD))
	{
        return BATTERY_HEALTH_TEMPERATURE_HIGH_OVERHIGH;
...
k3_bq24161_low_current_charge(struct k3_bq24161_device_info *di)
{
   ...
   if(!di->battery_present)
	...
	case BATTERY_HEALTH_TEMPERATURE_LOW:
        ...
        di->cin_limit = di->limit_currentmA;
	    break;
	...
	case BATTERY_HEALTH_TEMPERATURE_HIGH:
        ...
        k3_bq24161_config_safety_reg(di);
        di->enable_ce = ENABLE_CE;
		di->voltagemV = di->limit_voltagemV;
        if (di->charger_source == POWER_SUPPLY_TYPE_MAINS)
            di->cin_limit = CIN_LIMIT_800;
        else
            di->cin_limit = CURRENT_USB_LIMIT_IN;
	...
	case BATTERY_HEALTH_TEMPERATURE_OVERHIGH:
        ...
        if (di->charger_source == POWER_SUPPLY_TYPE_MAINS)
            ...
            di->cin_limit = CURRENT_USB_LIMIT_IN;
	    break;	
	case BATTERY_HEALTH_TEMPERATURE_HIGH_OVERHIGH:
        if (battery_voltage < BQ24161_PRECONDITIONING_BATVOLT_THRESHOLD) {
	        /*enable low charge,100mA charging*/
            di->enable_low_chg = ENABLE_LOW_CHG;
        } else {
...
static ssize_t k3_bq24161_show_limit_voltage(struct device *dev,
				  struct device_attribute *attr,
				  char *buf)
{
	unsigned long val;
...
static DEVICE_ATTR(limit_voltage, S_IWUSR | S_IRUGO,
				k3_bq24161_show_limit_voltage,
				NULL);
				 ...
				 const struct i2c_device_id *id)
{
    ...
    ret = bq24161_get_boardid_charge_parameter(di);
    if(!ret){
