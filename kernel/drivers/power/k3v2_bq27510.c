...
#include <linux/power/k3v2_bq24161.h>
#include <linux/power/k3v2_bq27510.h>
#include <linux/power/k3v2_battery_monitor.h>
...
int k3_bq27510_get_gpadc_conversion(int channel_no)
{
...
int k3_get_battery_id(void)
{
    ...
    battery_present = is_k3_bq27510_battery_exist(di);
    if(!battery_present){
        dev_info(di->dev, " battery is no present\n");
        return BAT_NO_PRESENT_STATUS;
    }
    data = k3_bq27510_get_gpadc_conversion(ADC_ADCIN3);
...
static bool k3_bq27510_get_gas_gauge_firmware_name(char *config_name)
{
    bool ret = 0;
...
static bool k3_bq27510_get_gasgauge_firmware_version_id(unsigned int* version_config)
{
    bool ret = 0;
...
static bool k3_bq27510_get_gasgauge_normal_capacity(unsigned int* design_capacity)
{
    bool ret = 0;
...
early_param("androidboot.swtype",early_parse_ate_enable_cmdline);

/* convert voltage to capacity */
#define vol_to_capacity(x)   ( (((x) -3400)* 100 /(4350-3400) /10 + 1)*10 )
#define VOLTAGE_SHUTDOWN  3450

int k3_bq27510_battery_capacity(struct k3_bq27510_device_info *di)
{
	int data 				= 0;
	int temp_voltage = VOLTAGE_SHUTDOWN;


    if(!bq27510_is_accessible())
    {
        printk(" function = [%s] bq27510 is not accessible ; capacity = %d \n",__FUNCTION__,gauge_context.capacity);
    ...
    if((data < 0)||(data > 100)) {
        BQ27510_ERR("i2c error in reading capacity! data = %d\n",data);
    ...
    else
	{
		if(factory_flag)
		{
			temp_voltage = k3_bq27510_battery_voltage(di);
			printk("function = [%s]  ; original capacity = %d ; voltage = %d; \n",__FUNCTION__,data,temp_voltage);
			/* when soc < 3,android will shut down */
			if( (data < 3) && (temp_voltage >= VOLTAGE_SHUTDOWN) )
			{
				data = vol_to_capacity(temp_voltage);
				data = (data <= 100) ? data : 100;
				printk("function = [%s]  ; changed capacity = %d ; voltage = %d ; \n ",__FUNCTION__,data,temp_voltage);
			}
		}
...
static int k3_bq27510_get_gasgauge_qmax(struct k3_bq27510_device_info *di)
{
...
static int k3_bq27510_check_qmax_property(void)
{
    int data = 0;
   ...
   if(!bq27510_is_accessible())
    ...
    data = k3_bq27510_get_gasgauge_qmax(di);
    if (!k3_bq27510_get_gasgauge_normal_capacity(&design_capacity))
...
static ssize_t bq27510_check_firmware_version(struct device_driver *driver, char *buf)
{
    ...
    battery_id_status = k3_get_battery_id();
    version = bq27510_get_firmware_version_by_i2c(g_battery_measure_by_bq27510_i2c_client);
	if (version < 0) {
        error = -STATE_HARDWARE_ERR;
    } else if (k3_bq27510_get_gasgauge_firmware_version_id(&version_config)) {
            if (version == version_config)
                error = 0;
    }
    return sprintf(buf,"%d",error);
}
static ssize_t k3_bq27510_check_qmax(struct device_driver *driver, char *buf)
{
    int error = 0;

    battery_id_status = k3_get_battery_id();
    error = k3_bq27510_check_qmax_property();
...
static int k3_bq27510_update_firmware(struct i2c_client *client, const char *pFilePath)
{
    ...
    int temp;
	/* open file */
    if (!k3_bq27510_get_gas_gauge_firmware_name(config_name)) {
        ...
        while (1) {
            ...
            if (strncmp(entry.file_name, config_name, strlen(config_name)) == 0) {
                ...
                if (strncasecmp(id, current_id, id_lenth) == 0) {
                    if((force_update_firmware_version_flag) || (!k3_bq27510_check_qmax_property())){
...
static ssize_t bq27510_show_gaugelog(struct device_driver *driver, char *buf)
{
    ...
    mdelay(2);
    ttf = k3_bq27510_i2c_read_word(g_battery_measure_by_bq27510_device,BQ27510_REG_II);
    mdelay(2);
    ...
    else
    {
      sprintf(buf, "%-9d  %-9d  %-4d  %-5d  %-6d  %-6d  %-6d  %-6d  0x%-5.4x  0x%-5.4x  %-6d  ",
                    voltage,  (signed short)cur, capacity, rm, fcc, (signed short)ttf, (signed short)si, temp, flag, control_status, ((qmax<<8)|qmax1) );
...
static DRIVER_ATTR(qmax, S_IRUGO, k3_bq27510_check_qmax,NULL);
...
static ssize_t k3_bq27510x_show_qmax(struct device *dev,
              struct device_attribute *attr,char *buf)
{
    unsigned long val;
    struct k3_bq27510_device_info *di = dev_get_drvdata(dev);

    val = k3_bq27510_get_gasgauge_qmax(di);
    return sprintf(buf, "%lu\n", val);
}
static ssize_t k3_bq27510x_show_normal_capacity(struct device *dev,
              struct device_attribute *attr,char *buf)
{
    unsigned long val;
    ...
    battery_id_status = k3_get_battery_id();
    k3_bq27510_get_gasgauge_normal_capacity(&normal_capacity);
    val = normal_capacity;
    return sprintf(buf, "%lu\n", val);
}
/*Firmware upgrade sysfs store interface*/
static ssize_t k3_bq27510_update_gasgauge_version(struct device *dev,
...
/* Firmware upgrade sysfs show interface*/
static ssize_t k3_bq27510_show_gasgauge_version(struct device *dev,
                  struct device_attribute *attr,
                  char *buf)
{
    int iRet = 0;
...
static ssize_t k3_bq27510_set_calculate_impedance(struct device *dev,
                  struct device_attribute *attr,
                  const char *buf, size_t count)
{
    long val;
...
static ssize_t k3_bq27510x_show_calculate_impedance(struct device *dev,
              struct device_attribute *attr,char *buf)
{
    unsigned long val;
...
static ssize_t k3_bq27510x_show_battery_id(struct device *dev,
              struct device_attribute *attr,char *buf)
{
    unsigned long val;

    val = k3_bq27510_get_gpadc_conversion(ADC_ADCIN3);

    return sprintf(buf, "%lu\n", val);
}
static ssize_t k3_bq27510_show_firmware_version(struct device *dev,
              struct device_attribute *attr,char *buf)
{
    int version = 0;
    int version_config = 0;
...
exit:
    if(error)
        ...
        battery_id_status = k3_get_battery_id();
        k3_bq27510_get_gasgauge_firmware_version_id(&version_config);
...
static DEVICE_ATTR(qmax, S_IWUSR | S_IRUGO,
                 k3_bq27510x_show_qmax,
                 NULL);
static DEVICE_ATTR(normal_capacity, S_IWUSR | S_IRUGO,
                  k3_bq27510x_show_normal_capacity,
                  NULL);
static DEVICE_ATTR(state, S_IWUSR | S_IRUGO,
                  k3_bq27510_show_gasgauge_version,
                  k3_bq27510_update_gasgauge_version);
static DEVICE_ATTR(battery_impedance, S_IWUSR | S_IRUGO,
                  k3_bq27510x_show_calculate_impedance,
                  k3_bq27510_set_calculate_impedance);
static DEVICE_ATTR(battery_id, S_IWUSR | S_IRUGO,
                  k3_bq27510x_show_battery_id,
                  NULL);
static DEVICE_ATTR(firmware_version, S_IWUSR | S_IRUGO,
                  k3_bq27510_show_firmware_version,
                  NULL);
