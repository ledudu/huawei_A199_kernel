...
#define DEBOUNCE_COUNT_LOOP     (3)  /*(2160MA*1%)/(2000MA/60/60)=39s*/
#define CHG_ODD_CAPACITY        (2)  /*capacity changing more than 5%*/
...
static int capacity_changed(struct bq_bci_device_info *di)
{
    ...
    if (curr_capacity != di->prev_capacity) {
      ...
      di->prev_capacity = curr_capacity;
      di->capacity_debounce_count = 0;
    } else if (++di->capacity_debounce_count >= DEBOUNCE_COUNT_LOOP) {
        /*only charge-work can not reach full(95%).Change capacity to full after 40min.*/
        if((di->charge_status == POWER_SUPPLY_STATUS_CHARGING) && (curr_capacity > CHG_CANT_FULL_THRESHOLD)){
