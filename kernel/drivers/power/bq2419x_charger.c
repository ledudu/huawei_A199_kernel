...
static void bq2419x_config_current_reg(struct bq2419x_device_info *di)
{
    ...
    if (currentmA < ICHG_MIN_500)
        currentmA = ICHG_MIN_500;
    else if(currentmA > ICHG_MAX_4532)
        currentmA = ICHG_MAX_4532;

    Vichrg = (currentmA - ICHG_MIN_500)/ICHG_STEP_64;

    di->charge_current_reg02 = (Vichrg << BQ2419x_ICHG_SHIFT);
                  ...
                  const char *buf, size_t count)
{
    ...
    if ((strict_strtol(buf, 10, &val) < 0) || (val < ICHG_MIN_500)
                   || (val > ICHG_3000))
        return -EINVAL;
                  ...
                  char *buf)
{
    ...
    read_reg[2] = di->temperature_15;
    read_reg[3] = di->temperature_warm;
    read_reg[4] = di->temperature_hot;

    sprintf(buf,"%-9d  %-9d  %-9d  %-9d  %-9d",
