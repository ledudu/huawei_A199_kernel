...
//uint8_t i=0;
	...
	do{
		...
		SiiMhlTxIncomingInfoFrameChanged(&aviInfoFrame);
		HalTimerWait(50); 
		TXC_DEBUG_PRINT(("aviInfoFrame.header.type_code:0x%02x,  i=%d************************\n",(uint16_t)aviInfoFrame.header.type_code, i));
		}while (( 0x82 != aviInfoFrame.header.type_code)&&( i <= 20));
#endif
    {
...
uint8_t numberThatWePruned=0;
	...
	#else
 if (limit)
    { numberThatWePruned=18;
        for (i = 0 ; i < limit -1 ; ++i)
        {
        TX_PRUNE_PRINT((" pDesc->dtd.pixelClockHigh: 0x%02X\n",(uint16_t) pDesc->dtd.pixelClockHigh));
            if ( (0x1e<= pDesc->dtd.pixelClockHigh)&&(0x20 > mhlTxConfig.devCapCache.aucDevCapCache[DEVCAP_OFFSET_MHL_VERSION]))
            {
            		//DetailedTimingDescriptor_t*)pDesc->dtd=0;
			memset(&pDesc->dtd, 0x00, sizeof(DetailedTimingDescriptor_t));
...
//uint8_t *pData = (uint8_t *)pInfoFrame;
    SiiOsDebugPrint(pszFile,iLine,SII_OSAL_DEBUG_TX, "MhlTx: %s: length:0x%02x -- ",pszId,length);
	for (j = 0; j < length; j++)
	{
        SiiOsDebugPrintSimple(SII_OSAL_DEBUG_TX, "%02X ", (uint8_t)pInfoFrame->body.avi.payLoad.hwPayLoad.ifData[j]);
		if (AT_ROW_END(j,32))
		{
