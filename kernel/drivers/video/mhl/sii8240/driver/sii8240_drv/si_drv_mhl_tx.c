...
//SiiRegWrite(REG_HPD_CTRL,0x55);
    upStreamHPD = 0;
	temp = SiiRegRead(REG_HPD_CTRL);
    SiiMhlTxNotifyUpStreamHPD(upStreamHPD);

	
    TX_DEBUG_PRINT(("Drv: Upstream HPD Acquired - driven Low.SiiRegRead(REG_HPD_CTRL)=0x%x\n",temp));
...
void SiiMhlTxDrvAcquireUpstreamHPDControlDriveHighImpl ( void )
{
   ...
   // temp = SiiRegRead(REG_HPD_CTRL);
    SiiMhlTxNotifyUpStreamHPD(upStreamHPD);	
    TX_DEBUG_PRINT(("Drv: Upstream HPD Acquired - driven high.SiiRegRead(REG_HPD_CTRL)=0x%x\n",temp));
...
void SiiMhlTxDeviceIsr (void)
{
	...
	{
#endif //)
		ERROR_DEBUG_PRINT(("SiiMhlTxDeviceIsr: fwPowerState = %d,g_modeFlags = %d\n",fwPowerState, g_modeFlags));
	  	#if 0
	...
	{
		SiiRegWrite(REG_INTR5, SiiRegRead(REG_INTR5));	// clear all interrupts
		SiiRegWrite(REG_INTR7, SiiRegRead(REG_INTR7));
		SiiRegWrite(REG_INTR9, SiiRegRead(REG_INTR9));
		SiiRegWrite(REG_INTR8, SiiRegRead(REG_INTR8));
		SiiRegWrite(REG_INTR1, SiiRegRead(REG_INTR1));
		SiiRegWrite(REG_TPI_INTR_ST0, SiiRegRead(REG_TPI_INTR_ST0));
		SiiRegWrite(REG_CBUS_INT_1, SiiRegRead(REG_CBUS_INT_1));
		SiiRegWrite(REG_CBUS_INT_0, SiiRegRead(REG_CBUS_INT_0));
	}
	ERROR_DEBUG_PRINT(("SiiMhlTxDeviceIsr end: fwPowerState = %d,g_modeFlags = %d\n",fwPowerState, g_modeFlags));
#ifdef EXHAUST_PENDING_INTERRUPTS_LINUX //(
		i++;
...
static void WriteInitialRegisterValuesPartTwo (void)
{
    ...
    //default value 0x20 SiiRegWrite(REG_DISC_CTRL7, 0x20);				// use 1K only setting
        ...
        SiiRegWrite(REG_PKT_FILTER_0, BIT_PKT_FILTER_0_DROP_GCP_PKT | BIT_PKT_FILTER_0_DROP_AVI_PKT | BIT_PKT_FILTER_0_DROP_MPEG_PKT | BIT_PKT_FILTER_0_DROP_CEA_GAMUT_PKT);
     		...
     		SiiRegModify(REG_HPD_CTRL,BIT6,1);	//"1" means Open drain // "0" means Push/Pull
    #endif
	...
	TX_DEBUG_PRINT(("Drv: SiiRegRead(REG_DISC_CTRL6) = 0x%02X,(int) SiiRegRead(REG_DISC_CTRL3)=0x%02X\n", (int) SiiRegRead(REG_DISC_CTRL6),(int) SiiRegRead(REG_DISC_CTRL3)));
#endif
}
...
static void ForceUsbIdSwitchOpen (void)
{
	...
	SiiRegWrite(REG_DISC_CTRL3, 0x83);//SiiRegWrite(REG_DISC_CTRL3, 0x8E);

	TX_DEBUG_PRINT(("Drv:ForceUsbIdSwitchOpen: SiiRegRead(REG_DISC_CTRL6) = 0x%02X,(int) SiiRegRead(REG_DISC_CTRL3)=0x%02X\n", (int) SiiRegRead(REG_DISC_CTRL6),(int) SiiRegRead(REG_DISC_CTRL3)));
#else
	DISABLE_DISCOVERY
...
static void ReleaseUsbIdSwitchOpen (void)
{
	...
	SiiRegModify(REG_DISC_CTRL6, BIT6, BIT6);
	SiiRegModify(REG_DISC_CTRL3, BIT3, 0x00);

	TX_DEBUG_PRINT(("Drv:ReleaseUsbIdSwitchOpen: SiiRegRead(REG_DISC_CTRL6) = 0x%02X,(int) SiiRegRead(REG_DISC_CTRL3)=0x%02X\n", (int) SiiRegRead(REG_DISC_CTRL6),(int) SiiRegRead(REG_DISC_CTRL3)));
#else
	SiiRegModify(REG_DISC_CTRL6, BIT6, BIT6);
	SiiRegModify(REG_DISC_CTRL3, BIT3, 0x00);
	HalTimerWait(50); // per spec
...
if(rgndImpedance == 0x03)
{
			...
			TX_DEBUG_PRINT(("Drv:RgndResultIsMhl: SiiRegRead(REG_DISC_CTRL6) = 0x%02X,(int) SiiRegRead(REG_DISC_CTRL3)=0x%02X\n", (int) SiiRegRead(REG_DISC_CTRL6),(int) SiiRegRead(REG_DISC_CTRL3)));
...
void SwitchToD0 (void)
{
	...
	temp = SiiRegRead(REG_INTR5);
	SiiRegWrite(REG_INTR5, temp);	// clear all interrupts
...
void SwitchToD3 (void)
{
	if(POWER_STATE_D3 != fwPowerState)
	{
		...
		Bits 1:0 set to 11*/
		SiiRegModify(TX_PAGE_2 | 0x0000, 0x03 ,0x03);
       ...
       // {
		...
		//SiiRegModify(REG_INTR5_MASK, BIT_INTR5_MHL_FIFO_UNDERFLOW|BIT_INTR5_MHL_FIFO_OVERFLOW|BIT_INTR5_CKDT_CHANGE|BIT_INTR5_SCDT_CHANGE , 0);
...
#else //)(
		if (!(temp& BIT_INTR4_RGND_DETECTION)){
			TXD_DEBUG_PRINT(("Drv: !(temp& BIT_INTR4_RGND_DETECTION)\n"));
			CLR_BIT(REG_DPD, 0);
			}
#endif //)
#ifdef INTERRUPT_DRIVEN_EDID //(
...
TXD_DEBUG_PRINT (("Drv: SiiRegRead(REG_INTR5_MASK):%x\n", (unsigned int)SiiRegRead(REG_INTR5_MASK)));
	SiiRegModify(REG_INTR5_MASK, BIT_INTR5_MHL_FIFO_UNDERFLOW|BIT_INTR5_MHL_FIFO_OVERFLOW|BIT_INTR5_CKDT_CHANGE|BIT_INTR5_SCDT_CHANGE ,
			BIT_INTR5_MHL_FIFO_UNDERFLOW|BIT_INTR5_MHL_FIFO_OVERFLOW|BIT_INTR5_CKDT_CHANGE|BIT_INTR5_SCDT_CHANGE );
...
void SiiMhlTxDrvSetUpstreamEDID(uint8_t *pEDID,uint16_t length)
{
	...
	SiiMhlTxDrvAcquireUpstreamHPDControlDriveLow(); //by wind
	//HalTimerWait(1000); 
	#ifdef K3_HDMI
	       k3_hdmi_enable_hpd(true);	
	       HalTimerWait(500); 
