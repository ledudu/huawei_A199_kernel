...
{
	u16 flag;
	printk("the host chip has received interrupt ! \n");
...
static int fm_power_down(struct fmdev *fmdev)
{
	...
	if (ret < 0)
		return ret;
	/* send the FM power down command */
	payload = 0x0;
	ret = fmc_send_cmd(fmdev, POWER_SET, REG_WR, &payload,
		sizeof(payload), NULL, NULL);
	if (ret < 0)
		return ret;
	fmdbg("FM power down command issued successfully !\n");
...
static int fm_power_up(struct fmdev *fmdev, u8 mode)
{
	...
	payload = FM_ENABLE;
	/* send the FM power up command */
	if (fmc_send_cmd(fmdev, POWER_SET, REG_WR, &payload,
			sizeof(payload), NULL, NULL))
		goto rel;
	/* Allow the chip to settle down in Channel-8 mode */
	fmdbg("FM power up command issued successfully !\n");
	msleep(20);
