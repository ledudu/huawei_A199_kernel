...
int fm_rx_set_freq(struct fmdev *fmdev, u32 freq)
{
	...
	if (ret < 0)
		goto exit;
	printk("Tune channels command has been issued successfully !\n");
	/* Wait for tune ended interrupt */
	init_completion(&fmdev->maintask_comp);
	printk("waiting for the interrupt from the FM chip......\n");
	...
	printk("after tune channel, current frequency is %d \n ", curr_frq_in_khz);
	/* Update local cache  */
	...
	if (comp_scan == COMP_SCAN_STOP) {
		...
		if (fmdev->rx.comp_scan_status == COMP_SCAN_START &&
			...
			if (ret < false)
			    ...
			    fmdbg("cancel search command issued successfully !\n");
		}
...
again:
	...
	if (comp_scan == SEEK_START) {
		...
		if (ret < false)
			return ret;
		printk("Seek channels command has been issued successfully !\n");
		...
		if (ret < false)
			return ret;
		printk("Scan channels command has been issued successfully !\n");
