...
static void k3_vibrator_onoff_handler(struct work_struct *data)
{
	...
	if (on) {
		...
		if(motor_type==1)
		{
			tmp=readl(PMU_LDO12_CTRL);
			tmp=(tmp & ~ HI6421_LDO_ENA_MASK) | (HI6421_LDO_ENA & HI6421_LDO_ENA_MASK);
			writel(tmp,PMU_LDO12_CTRL);
		}
		else
		{
		...
		{
			tmp=readl(PMU_LDO12_CTRL);
			tmp=(tmp & ~ HI6421_LDO_ENA_MASK) | (0 & HI6421_LDO_ENA_MASK);
			writel(tmp,PMU_LDO12_CTRL);
		}
		else
		{
