...
#define CAM_VCC_PIN "gpio_073"
#define SCAM_VCC_PIN "gpio_173"
#define CAM_ID_PIN "gpio_111"
#define SCAM_ID_PIN "gpio_112"

#define MCAM_VCC_EN GPIO_9_1
#define SCAM_VCC_EN GPIO_21_5
#define MAIN_CAMERA_ID GPIO_13_7
#define SLAVE_CAMERA_ID GPIO_14_0
...
/*
 * Focus driver ic dw9714 information:
 * Foxconn ov8850 use this driver ic
 */
vcm_info_s vcm_dw9714_ov8850_f = {
	.vcm_type = VCM_DW9714_OV8850_F,
	.offsetInit = 0, /* maybe different in each AF mode */
	.fullRange = 1023, /* maybe different in each AF mode */
...
EXPORT_SYMBOL(vcm_dw9714_ov8850_f);

/*
 * Focus driver ic dw9714 information:
 * Liteon s5k4h5 use this driver ic
 */
vcm_info_s vcm_dw9714_s5k4h5_l = {
	.vcm_type = VCM_DW9714_S5K4H5_L,
	.offsetInit = 0, /* maybe different in each AF mode */
	.fullRange = 1023, /* maybe different in each AF mode */
...
EXPORT_SYMBOL(vcm_dw9714_s5k4h5_l);
...
int camera_power_core_ldo(camera_power_state power)
{
	...
	if (POWER_ON == power) {
		...
		ret = gpio_request(MCAM_VCC_EN, "mcam_vcc_en");
		if (ret < 0)
			print_error("fail to get MCAM_VCC_EN:%d", MCAM_VCC_EN);
		ret = gpio_direction_output(MCAM_VCC_EN, 1);
		if (ret < 0)
			print_error("fail to set MCAM_VCC_EN:%d to 1", MCAM_VCC_EN);
        
		ret = pinmux_setpullupdown(gpio_slave_core_ldo, NOPULL);
		if (ret < 0)
			print_error("fail to set gpio_slave_core_ldo to NOPULL");
		ret = gpio_request(SCAM_VCC_EN, "scam_vcc_en");
		if (ret < 0)
			print_error("fail to get GPIO_21_5");
		ret = gpio_direction_output(SCAM_VCC_EN, 1);
		if (ret < 0)
			print_error("fail to set GPIO_21_5 to 1");
	} else {
		print_info("power off core ldo");
		ret = gpio_direction_output(MCAM_VCC_EN, 0);
		if (ret < 0)
			print_error("fail to set MCAM_VCC_EN:%d to 0", MCAM_VCC_EN);
		gpio_free(MCAM_VCC_EN);
		ret = gpio_direction_output(SCAM_VCC_EN, 0);
		if (ret < 0)
			print_error("fail to set GPIO_21_5 to 0");
		gpio_free(SCAM_VCC_EN);
	}
...
int camera_power_id_gpio(camera_power_state power)
{
	...
	print_debug("enter %s:power = %d", __func__, power);
	...
	if (POWER_ON == power) {
		...
		if (NULL == gpio_main_camera_id) {
			...
			if (!gpio_main_camera_id) {
				print_error("fail to get CAM_ID_PIN:%d", CAM_ID_PIN);
				return -EINVAL;
			}
		}
		...
		ret = gpio_request(MAIN_CAMERA_ID, NULL);
		if (ret < 0)
			print_error("fail to get MAIN_CAMERA_ID:%d", MAIN_CAMERA_ID);
		ret = gpio_direction_input(MAIN_CAMERA_ID);
		if (ret < 0) {
			print_error("fail to set MAIN_CAMERA_ID:%d input", MAIN_CAMERA_ID);
		} else {
			ret = gpio_get_value(MAIN_CAMERA_ID);
			if (ret < 0)
				print_error("fail to get MAIN_CAMERA_ID:%d value", MAIN_CAMERA_ID);
		...
		if (NULL == gpio_slave_camera_id) {
			...
			if (!gpio_slave_camera_id) {
				print_error("fail to get SCAM_ID_PIN:%d", SCAM_ID_PIN);
				return -EINVAL;
			}
		}
		...
		ret = gpio_request(SLAVE_CAMERA_ID, NULL);
		if (ret < 0)
			print_error("fail to get SLAVE_CAMERA_ID:%d", SLAVE_CAMERA_ID);
		ret = gpio_direction_input(SLAVE_CAMERA_ID);
		if (ret < 0)
			print_error("fail to set SLAVE_CAMERA_ID:%d input", SLAVE_CAMERA_ID);
