...
static int ispv1_setreg_vcm_code_done(vcm_info_s *vcm, u32 vcm_code)
{
	...
	} else if (vcm->vcm_type == VCM_DW9714_SS || vcm->vcm_type == VCM_DW9714_Sunny || vcm->vcm_type == VCM_DW9714_Liteon || VCM_DW9714_OV8850_F == vcm->vcm_type || VCM_DW9714_S5K4H5_L == vcm->vcm_type ) {
		/*in dw9714 case, ad value must set to bit4 ~ bit13*/
...
static bool ispv1_setreg_vcm_code(u32 dest_code)
{
	...
	if ((dest_code < safe_position) && (curr_code > safe_position)
		...
		if (vcm->vcm_type == VCM_AD5823) {
			...
			max_step_cnt = 2;
		}  else if ((vcm->vcm_type == VCM_DW9714_SS || vcm->vcm_type == VCM_DW9714_Sunny || vcm->vcm_type == VCM_DW9714_Liteon || VCM_DW9714_OV8850_F == vcm->vcm_type || VCM_DW9714_S5K4H5_L == vcm->vcm_type ) && dest_code == vcm->offsetInit){
		...
		if (vcm->vcm_type == VCM_DW9714_SS || vcm->vcm_type == VCM_DW9714_Sunny || vcm->vcm_type == VCM_DW9714_Liteon || VCM_DW9714_OV8850_F == vcm->vcm_type || VCM_DW9714_S5K4H5_L == vcm->vcm_type )
			ispv1_setreg_vcm_DLC_mode();
		ret = ispv1_setreg_vcm_code_done(vcm, dest_code);
	}
...
static int ispv1_setreg_vcm_DLC_mode()
	...
	print_info("enter %s", __func__);
		...
		if (vcm->vcm_type == VCM_DW9714_SS) {
			...
			ret |= ispv1_write_vcm(vcm->vcm_id, vcm->moveLensAddr[0], 0xA10D, I2C_16BIT, SCCB_BUS_MUTEX_NOWAIT);
		} else if ( VCM_DW9714_OV8850_F == vcm->vcm_type ){
			ret |= ispv1_write_vcm(vcm->vcm_id, vcm->moveLensAddr[0], 0xF290, I2C_16BIT, SCCB_BUS_MUTEX_NOWAIT);
			ret |= ispv1_write_vcm(vcm->vcm_id, vcm->moveLensAddr[0], 0xA10E, I2C_16BIT, SCCB_BUS_MUTEX_NOWAIT);
		}else if ( VCM_DW9714_S5K4H5_L == vcm->vcm_type ){
			ret |= ispv1_write_vcm(vcm->vcm_id, vcm->moveLensAddr[0], 0xF208, I2C_16BIT, SCCB_BUS_MUTEX_NOWAIT);
			ret |= ispv1_write_vcm(vcm->vcm_id, vcm->moveLensAddr[0], 0xA10D, I2C_16BIT, SCCB_BUS_MUTEX_NOWAIT);
		}
			ret |= ispv1_write_vcm(vcm->vcm_id, vcm->moveLensAddr[0], 0xDC51, I2C_16BIT, SCCB_BUS_MUTEX_NOWAIT);
...
bool ispv1_judge_skip_frame(void)
{
	...
	print_info("expo_line:%d, vts:%d, height:%d, expo time:%d, stable_expo_ms:%d ret:%d, fps:%d",
		get_writeback_expo()>>4, vts, height, expo_time_ms, stable_expo_ms, ret, sensor->fps);
	return ret;
}
...
void ispv1_k3focus_run(void)
{
	...
	while ((schedule_case = ispv1_focus_need_schedule()) != -1) {
		ret = ispv1_wait_focus_schedule_timeout(150);
		if (ret != 0)
			continue;
