...
#endif
static int file_open_firmware(struct file **filp, u8 **buf)
{
...
static void file_close_firmware(struct file **fw_entry, u8 **fw_data)
{
    filp_close(*fw_entry, NULL);
    vfree(*fw_data);
}
...
//			img_name ? img_name : data.product_id);
	...
	if (fw_entry)
	{
		file_close_firmware(&fw_entry, &fw_data);
		if(!ret)
