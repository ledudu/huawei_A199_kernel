...
u16 isp_mwb_gain[CAMERA_WHITEBALANCE_MAX][3] = {
	...
	{0x0119, 0x0080, 0x0098}, /* INCANDESCENT 2800K */
	{0x00f0, 0x0080, 0x00c4}, /* FLUORESCENT 4200K */
	{0x00a0, 0x00a0, 0x00a0}, /* WARM_FLUORESCENT, y36721 todo */
	{0x00d1, 0x0080, 0x00d2}, /* DAYLIGHT 5000K */
	{0x00a0, 0x0080, 0x0100}, /* CLOUDY_DAYLIGHT 6500K*/
...
static u8 denoise[27];
static u8 sharpness[2];
static bool denoise_already_closed_flg = false;
...
int ispv1_set_hwscope(int ctl)
{
	print_debug("enter %s, %d", __func__, ctl);

	if(ctl == 1)    //disable denoise
	{
		...
		else
		{
			...
			//sonyimx135.c
			denoise[0] = GETREG8(0x6551a);
			denoise[1] = GETREG8(0x6551b);
			denoise[2] = GETREG8(0x6551c);
			denoise[3] = GETREG8(0x6551d);
			denoise[4] = GETREG8(0x6551e);
			denoise[5] = GETREG8(0x6551f);
			denoise[6] = GETREG8(0x65520);
			denoise[7] = GETREG8(0x65522);
			denoise[8] = GETREG8(0x65523);
			denoise[9] = GETREG8(0x65524);
			denoise[10] = GETREG8(0x65525);
			denoise[11] = GETREG8(0x65526);
			denoise[12] = GETREG8(0x65527);
			denoise[13] = GETREG8(0x65528);
			denoise[14] = GETREG8(0x65529);
			denoise[15] = GETREG8(0x6552a);
			denoise[16] = GETREG8(0x6552b);
			denoise[17] = GETREG8(0x6552c);
			denoise[18] = GETREG8(0x6552d);
			denoise[19] = GETREG8(0x6552e);
			denoise[20] = GETREG8(0x6552f);

			denoise[21] = GETREG8(0x65c00);
			denoise[22] = GETREG8(0x65c01);
			denoise[23] = GETREG8(0x65c02);
			denoise[24] = GETREG8(0x65c03);
			denoise[25] = GETREG8(0x65c04);
			denoise[26] = GETREG8(0x65c05);

			sharpness[0] = GETREG8(0x6560c);
			sharpness[1] = GETREG8(0x6560d);
	...
	else if(ctl == 2)    //recover denoise
	{
		...
		//sonyimx135.c
		SETREG8(0x6551a, denoise[0]);
		SETREG8(0x6551b, denoise[1]);
		SETREG8(0x6551c, denoise[2]);
		SETREG8(0x6551d, denoise[3]);
		SETREG8(0x6551e, denoise[4]);
		SETREG8(0x6551f, denoise[5]);
		SETREG8(0x65520, denoise[6]);
		SETREG8(0x65522, denoise[7]);
		SETREG8(0x65523, denoise[8]);
		SETREG8(0x65524, denoise[9]);
		SETREG8(0x65525, denoise[10]);
		SETREG8(0x65526, denoise[11]);
		SETREG8(0x65527, denoise[12]);
		SETREG8(0x65528, denoise[13]);
		SETREG8(0x65529, denoise[14]);
		SETREG8(0x6552a, denoise[15]);
		SETREG8(0x6552b, denoise[16]);
		SETREG8(0x6552c, denoise[17]);
		SETREG8(0x6552d, denoise[18]);
		SETREG8(0x6552e, denoise[19]);
		SETREG8(0x6552f, denoise[20]);

		SETREG8(0x65c00, denoise[21]);
		SETREG8(0x65c01, denoise[22]);
		SETREG8(0x65c02, denoise[23]);
		SETREG8(0x65c03, denoise[24]);
		SETREG8(0x65c04, denoise[25]);
		SETREG8(0x65c05, denoise[26]);

		SETREG8(0x6560c, sharpness[0]);
		SETREG8(0x6560d, sharpness[1]);
	...
	print_debug("exit %s, %d", __func__, ctl);
	return 0;
}
...
/*
 *dynamic denoise func is not good and will be rebuild in furture
 */
void ispv1_dynamic_ydenoise(camera_sensor *sensor, camera_state state, bool flash_on)
{
	...
	u8 ydenoise_value[ ] = {
		0x02, /* gain 1x y denoise */
		0x04, /* gain 2x y denoise */
		0x0a, /* gain 4x y denoise */
		0x10, /* gain 8x y denoise */
		0x16, /* gain 16x y denoise */
		0x16,/* gain 32x y denoise */
		0x16,/* gain 64x y denoise */
	};
	u16 uvdenoise_value[ISP_DENOISE_ARRAY_SIZE] = {
		0x08, /* gain 1x uv denoise */
		0x10, /* gain 2x uv denoise */
		0x20, /* gain 4x uv denoise */
		0x20, /* gain 8x uv denoise */
		0x20, /* gain 16x uv denoise */
		0x20,/* gain 32x uv denoise */
		0x20,/* gain 64x uv denoise */
	};

	if (state == STATE_PREVIEW) {
		//y denoise
		ydenoise_value[0] = 0x02;
		ydenoise_value[1] = 0x04;
		ydenoise_value[2] = 0x04;
		ydenoise_value[3] = 0x04;
		ydenoise_value[4] = 0x04;
		ydenoise_value[5] = 0x04;
		ydenoise_value[6] = 0x04;
		//uv denoise
		uvdenoise_value[0] = 0x0c;
		uvdenoise_value[1] = 0x18;
		uvdenoise_value[2] = 0x28;
		uvdenoise_value[3] = 0x28;
		uvdenoise_value[4] = 0x28;
		uvdenoise_value[5] = 0x28;
		uvdenoise_value[6] = 0x28;
	} 
	else
	{
		//index = sensor->capture_frmsize_index;
		if (flash_on == false) 
		{
			...
			if (ae_value <= ae_th[0]) 
			{
				ydenoise_value[0] = 0x04;
				ydenoise_value[1] = 0x06;
			} 
			else 
			{
				ydenoise_value[0] = 0x06;
				ydenoise_value[1] = 0x06;
			}
		} 
		else 
		...
		ydenoise_value[2] = 0x0a;
		ydenoise_value[3] = 0x10;
		ydenoise_value[4] = 0x16;
		ydenoise_value[5] = 0x16;
		ydenoise_value[6] = 0x16;
		//uv denoise
		uvdenoise_value[0] = 0x08;
		uvdenoise_value[1] = 0x10;
		uvdenoise_value[2] = 0x20;
		uvdenoise_value[3] = 0x20;
		uvdenoise_value[4] = 0x20;
		uvdenoise_value[5] = 0x20;
		uvdenoise_value[6] = 0x20;
	}
