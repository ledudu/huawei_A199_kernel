...
static irqreturn_t pn544_dev_irq_handler(int irq, void *dev_id)
{
	...
	//printk("----------enter pn544_dev_irq_handler------ %d\n", irq);
		...
		size_t count, loff_t *offset)
{
	...
	//printk("%s : start to gpio_get_value with got value: %d\n", __func__, gpio_get_value(pn544_dev->irq_gpio));
	if (!gpio_get_value(pn544_dev->irq_gpio)) {
		if (filp->f_flags & O_NONBLOCK) {
			printk("--->Lintar flip->f_flags=%u\n",filp->f_flags);
			ret = -EAGAIN;
			goto fail;
		}
		
		pn544_dev->irq_enabled = true;
		//printk("%s : start to enable_irq.\n", __func__);
		enable_irq(pn544_dev->client->irq);
		//printk("%s : start to wait_event_interruptible.\n", __func__);
		ret = wait_event_interruptible(pn544_dev->read_wq,
				gpio_get_value(pn544_dev->irq_gpio));
		//printk("%s : end to pn544_disable_irq, with ret: %d.\n", __func__, ret);
	...
	//printk("%s : start to i2c_master_recv.\n", __func__);
	/* Read data */
	ret = i2c_master_recv(pn544_dev->client, tmp, count);

	mutex_unlock(&pn544_dev->read_mutex);

	//printk("%s : end to i2c_master_recv, with the ret: %d.\n", __func__, ret);
		...
		size_t count, loff_t *offset)
{
	...
	/* Write data */
	    ...
	    if (ret != count) 
	    {
	         printk("%s : send data try =%d returned %d\n", __func__,calc,ret);
	         continue;
	    }
	    else
		break;
...
static int pn544_dev_open(struct inode *inode, struct file *filp)
{
    ...
    printk("----------enter pn544_dev_open -------------\n");

    filp->private_data = pn544_dev;

	printk("%s : %d,%d\n", __func__, imajor(inode), iminor(inode));

	printk("----------exit pn544_dev_open -------------\n");
		...
		const struct i2c_device_id *id)
{
	...
	client->irq = gpio_to_irq(client->irq);//hefei add
