...
/* set minimum value of lcd brightness */
//add pwm duty cycle limit
#define BL_MIN_LEVEL 10
#define BL_MAX_LEVEL 250
...
/* Add esd detect scheme in overlay compose function */
        ...
        if (cmd_mode) {
            ...
            if (wait_ret < 0 || k3fd->esd_recover == true) {
                if (k3fd->panel_info.esd_enable) {
					...
					k3_fb_set_backlight(k3fd, bl_level);
...
/* Add esd detect scheme in overlay compose partial function */
...
static int k3fb_frc_get_fps(struct k3_fb_data_type *k3fd)
{
	...
	if (k3fd->panel_info.type == PANEL_MIPI_CMD) {
        ...
        /*Base on the different scene to set the lcd frame rate*/
...
static void k3_fb_set_backlight_cmd_mode(struct k3_fb_data_type *k3fd, u32 bkl_lvl)
{
	...
	if (k3fd->cmd_mode_refresh)
		return;
	/* limit the value of minimum lcd brightness */
	//add pwm duty cycle limit
	if (bkl_lvl)
	{
		if (bkl_lvl > BL_MAX_LEVEL)
		{
			bkl_lvl = BL_MAX_LEVEL;
		}
		else if (bkl_lvl < BL_MIN_LEVEL)
		{
			bkl_lvl = BL_MIN_LEVEL;
		}
	}
...
/* write backlight brightness after lcd recover */
