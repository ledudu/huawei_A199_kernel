...
static int setup_dvc_and_phy(void)
{
	...
	case E_USBPHY_TUNE_PLATFORM:
		reg_value |= 0x23;
		break;
    /* add USBPHY config for G710C, the parameter is 0x017A277 */
    case E_USBPHY_TUNE_G710C:
        reg_value |= 0x03;          //txrisetun, 11:-40% rise and fall time
        reg_value |= (0x0D << 2);   //txvreftune, 1101:+20% high-speed DC level voltage
        reg_value |= (0x03 << 15);  //txrestune, 11: Source impedance is decreased by approximately 4 ohm
        reg_value |= (0x03 << 17);  //txpreempamptune, 11:3X
        reg_value &= ~(0x03 << 6);
        reg_value |= (0x01 << 6);   //txhsxvtune, 01:-15 mV crosspoint
        break;
