...
#define COMPASS_RST_PIN "gpio_116"
#define COMPASS_RST_GPIO_NUM 116
...
int akm8963_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    /* add GPI/O source request function */

    err = gpio_request(COMPASS_RST_GPIO_NUM, "COMPASS_RST_GPIO");
    if (err) {
        dev_err(&client->dev, "failed to request gpio %d for compass rst\n", COMPASS_RST_GPIO_NUM);
        goto exit2;
    }

	err = akm8963_gpio_init(PULLUP, COMPASS_RST_PIN);	//set the GPIO_116 PULLUP
	if (err < 0) {
		dev_err(&client->dev, "COMPASS_RST_PIN failed to init gpio\n");
		goto exit3;
	}
	gpio_direction_output(COMPASS_RST_GPIO_NUM, 1);	//set the GPIO_116 output

	err = akm8963_gpio_init(PULLDOWN, MSENSOR_PIN);
	if (err < 0) {
		dev_err(&client->dev, "COMPASS_IRQ_PIN failed to init gpio\n");
		goto exit3;
	}
...
exit4:
    ...
    /* add GPI/O source free function */

exit3:
    gpio_free(COMPASS_RST_GPIO_NUM);
