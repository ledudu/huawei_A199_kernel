...
static struct pdt_entry f34_pdt, f01_pdt;
static bool has_f34 = false;
struct rmi_device *rmi_update;
#endif
...
static ssize_t update_firmware_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (1 != sscanf(buf, "%d", &regs)) {
		printk("update_firmware_store err to get values\n");
		return -EINVAL;
	}
	if (regs)
	{
		if (has_f34)
		{
			rmi4_fw_update(rmi_update, &f01_pdt, &f34_pdt);
			ret = 0;
		} 
		else
			printk("WARNING: No F34, firmware update will not be done.\n");
	}
	return count;
 }
static struct kobj_attribute update_firmware_attribute = {
	.attr = {.name = "update_rmi_firmware", .mode = 0664},
	.show = update_firmware_show,
	.store = update_firmware_store,
};
static int rmi_firmware_file(void)
{
	int ret;
	struct kobject *kobject_rmi;
	kobject_rmi = kobject_create_and_add("touch_screen", NULL);
	if (!kobject_rmi)
	{
		printk("create kobjetct error!\n");
		return -1;
	}
	ret = sysfs_create_file(kobject_rmi, &update_firmware_attribute.attr);
	if (ret) {
		kobject_put(kobject_rmi);
