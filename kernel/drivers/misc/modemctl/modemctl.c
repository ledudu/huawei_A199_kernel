...
static long modemctl_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case MODEMCTL_SWITCH_TO_FIRST:
        ...
        gpio_set_value(pdata->gpio_modemctl_en, 0);
        if(product_type("G710C"))
        {
            gpio_set_value(MTK_SWITCH_GPIO, 0);
        }
        break;
    case MODEMCTL_SWITCH_TO_SECOND:
        ...
        if(product_type("G710C"))
        {
            gpio_set_value(MTK_SWITCH_GPIO, 1);
        }
        ret = blockmux_set(modemctl_iomux_block, modemctl_block_config, LOWPOWER);
        if (0 > ret) {
            loge("%s: set iomux to gpio lowpower error", __FUNCTION__);
...
static int __devinit modemctl_probe(struct platform_device *pdev)
{
    ...
    {
	    /* request gpio */
        ret = gpio_request(MTK_SWITCH_GPIO, "MTK_SWITCH_GPIO43");
        if (0 > ret) 
		{
            loge("%s: MTK_SWITCH_GPIO request enable pin failed", __FUNCTION__);
            return ret;
        }
        ret = gpio_direction_output(MTK_SWITCH_GPIO, 0);
        if (0 > ret) 
		{
            loge("%s: set MTK_SWITCH_GPIO 43 failed", __FUNCTION__);
            gpio_free(MTK_SWITCH_GPIO);
            return ret;
        }
    }
