...
/*CONFIG_BATTERY_K3V2 define for G710 charge ,bq24161*/
#ifndef CONFIG_BATTERY_K3V2
...
struct cyttsp4_sett_param_map cyttsp4_config_param_map[] = {
	...
	[0] = {
			  .id = 0,
			  .param = &cyttsp4_sett_ofilm_param_regs,
		  },
...
/*add default config, is null, not update*/
struct cyttsp4_sett_param_map cyttsp4_config_param_map[] = {
...
#define GPIO_LCD_ID0	(129)
#define GPIO_LCD_ID1	(130)
#define GPIO_LCD_TE (072)
#define GPIO_PWM0   (142)  //disconnect
#define GPIO_PWM1   (143)  //disconnect
...
/* add cmi lcd driver */
static struct platform_device mipi_cmd_otm1282a_hd = {
...
static void __init virtualkeys_init(void)
{
	...
	buf_vkey_size = sprintf(buf_virtualkey,
                __stringify(EV_KEY) ":" __stringify(KEY_BACK)  ":120:1340:200:100"
                 ":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE)   ":360:1340:200:100"
                 ":" __stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":600:1340:200:100"
                 "\n");
		...
		int on, struct device *dev)
{
	...
	int rc = 0;
	struct iomux_pin *tp_inter;


	tp_inter=iomux_get_pin("gpio_157");

	pinmux_setpullupdown(tp_inter, PULLUP);
...
static struct cyttsp4_core_platform_data _cyttsp4_core_platform_data = {
    ...
    /* active the sys file for glove function */    
...
#endif
/*irda gpio setup*/
static int irda_gpio_setup (void *gpio_data, bool configure) {
        int retval = 0;

	 retval = gpio_request(IRDA_RESET, "irda_reset");
        if (retval) {
            pr_err("%s: Failed to get reset gpio %d. Code: %d.",
                   __func__, IRDA_RESET, retval);
            return retval;
        }

        retval = gpio_direction_output(IRDA_RESET,0);
        if (retval) {
            pr_err("%s: Failed to setup reset gpio %d. Code: %d.",
                   __func__, IRDA_RESET, retval);
            gpio_free(IRDA_RESET);
        }

      msleep (100);
	 retval = gpio_direction_output(IRDA_RESET,1);
        if (retval) {
            pr_err("%s: Failed to setup reset gpio %d. Code: %d.",
                   __func__, IRDA_RESET, retval);
            gpio_free(IRDA_RESET);
        ...
        pr_warn("%s: No way to deconfigure gpio %d and %d.",
               __func__, IRQ_GPIO_022,IRDA_RESET);
      }
      return retval;
}

static struct irda_device_platorm_data irda_platformdata = {
       .en_gpio = IRDA_2V85EN,
       .gpio_config = irda_gpio_setup,
...
/* add the TPA2015_SPK platform device */
static struct tpa2015_platform_data tpa2015_pdata = {
    .gpio_tpa2015_en    = GPIO_17_3,/* 139 */
};

static struct platform_device tpa2015_device = {
	.name    = TPA2015_NAME,
	.id      = 0,
	.dev     = {
		.platform_data = &tpa2015_pdata,
	},
};
/* SPK_5VBOOST */
...
static struct lm3642_platform_data lm3642_platform_data =
{
	//.reset_pin			= GPIO_9_4,
	.strobe0			= GPIO_8_1,
	//.strobe1			= GPIO_8_2,
...
#ifdef CONFIG_HIK3_CAMERA_FLASH
	[2]	=	{
		.type			= K3_FLASH_LM3642_NAME,
		.addr			= K3_FLASH_LM3642_I2C_ADDR,
		.platform_data		= &lm3642_platform_data,
	},
#endif
...
static struct i2c_board_info hisik3_i2c_bus3_devs[]= {
	...
	[2]	=	{
		...
		.addr			= 0x4D,
		.flags 			= true,
		.platform_data 	= &irda_platformdata,
	},
};
...
static void k3v2_i2c_devices_init(void)
{
	...
	if(get_spk_pa(str_tpa2028) == true) {
	    printk(KERN_INFO" spk_tpa :k3v2oem1_init k3v2oem2  str_tpa2028!\n");
...
static void __init k3v2oem1_init(void)
{
	...
	const char str_tpa2015[] = "TPA2015";
	edb_trace(1);
	k3v2_common_init();
	/*
	...
	if(get_spk_pa(str_boost5v) == true) {
	    printk(KERN_INFO" spk_tpa :k3v2oem1_init k3v2oem2  str_boost5v!\n");
		platform_device_register(&spk_5vboost_device);
	}
	if(get_spk_pa(str_tpa2015) == true) {
	    printk(KERN_INFO" spk_tpa :k3v2oem1_init k3v2oem2  str_tpa2015!\n");
		platform_device_register(&tpa2015_device);
	}
