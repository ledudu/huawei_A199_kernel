...
const struct isp_reg_t isp_init_regs_sonyimx188[] = {
	...
	{0x1c140, 0x01},//banding,enable/disable the banding filters, 0:disable; 1:enable
	{0x1c13e, 0x02},//real gain mode for OV8830
	
	{0x66401, 0x00},//window weight
	{0x66402, 0x80},//StatWin_Left
	{0x66403, 0x00},
	{0x66404, 0x80},//StatWin_Top
	{0x66405, 0x00},
	{0x66406, 0x80},//StatWin_Right
	{0x66407, 0x00},
	{0x66408, 0x80},//StatWin_Bottom           
	...
	{0x66501, 0x00},
	{0x66502, 0x8f},//0xff
	{0x66503, 0x0b},//0x0f,the percentage of pixels in the image that are clipped to black
	{0x1c1b0, 0xff},
	{0x1c1b1, 0xff},
	{0x1c1b2, 0x01},
	{0x65905, 0x08},
	...
	{0x1c5a0, 0x40},//AE target high, should close
	...
	//0x6551a~0x65520 for long exposure,One for Green channel
	...
	{0x6551f, 0x30},// 32X
	{0x65520, 0x40},// 64X
	//Another one is for red-green and blue-green channels
	...
	/* Global Gamma */
	{0x1c49b, 0x01}, //enable/disable manual gamma with 1: enable and 0: disable
	{0x1c49c, 0x02},
	...
	/* LENC */
	{0x1c247, 0x01},//three profile,color temperature based lens shading correction mode 1: enable 0: disable
	{0x1c24c, 0x00},
	{0x1c24d, 0x40},
	{0x1c24e, 0x00},
	{0x1c24f, 0x80},
	...
	{0x1cccf, 0x3f},//B gain for A
	{0x1ccd0, 0x00},//R gain for A
	{0x1c5b8, 0x10},//B gain for C outdoor Richard@0517
	{0x1c5b9, 0x20},//R gain for C outdoor Richard@0517
	...
	{0x1ccd4, 0x0f},//R gain for D indoor
	{0x1cccc, 0x00},//B gain for D outdoor
	{0x1cccd, 0x0f},//R gain for D outdoor
	...
	{0x1ccd5, 0x36}, //CT_A, 2012.06.02 yuanyabin
	{0x1ccd6, 0x5b}, //CT_C
	{0x1ccd7, 0xb9}, //CT_D
	...
	{0x6620e, 0x33}, //A Limit
	{0x6620f, 0x80}, //D65 split
...
static framesize_s sonyimx188_framesizes[] = {
	{0, 14, 1280, 800, 3000, 900, 30, 25, 0x10A, 0xDE, 0x79, VIEW_FULL, RESOLUTION_4_3, false, {sonyimx188_framesize_full_30fps, ARRAY_SIZE(sonyimx188_framesize_full_30fps)} },
};
...
static int sonyimx188_init_reg(void)
{
	...
	size = ARRAY_SIZE(isp_init_regs_sonyimx188);
	sonyimx188_write_isp_seq(isp_init_regs_sonyimx188, size);
...
static int sonyimx188_update_flip(u16 width, u16 height)
{
	...
	if (sonyimx188_sensor.old_flip != new_flip) {
		k3_ispio_update_flip((sonyimx188_sensor.old_flip ^ new_flip) & 0x03, width, height, SONYIMX188_PIXEL_ORDER_CHANGED);
...
static void sonyimx188_set_default(void)
{
	...
	sonyimx188_sensor.sensor_gain_to_iso = sonyimx188_gain_to_iso;
	sonyimx188_sensor.sensor_iso_to_gain = sonyimx188_iso_to_gain;
	...
	if (chip_id == DI_CHIP_ID) {
		for (i = 0; i < (ARRAY_SIZE(sonyimx188_framesizes)); i++)
			sonyimx188_framesizes[i].fps = sonyimx188_framesizes[i].fps_es;

		/* full size, add by y36721 */
		i = ARRAY_SIZE(sonyimx188_framesizes) - 1;
		sonyimx188_framesizes[i].sensor_setting.setting = sonyimx188_framesize_full_es;
		sonyimx188_framesizes[i].sensor_setting.seq_size = ARRAY_SIZE(sonyimx188_framesize_full_es);
	}
#endif
}
