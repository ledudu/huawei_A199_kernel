...
static int apds990x_debug_mask = 0;
module_param_named(apds990x_debug, apds990x_debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP);
/*echo 1 > /sys/module/apds990x/parameters/apds990x_debug open debug*/
static int apds_990x_pwindows_value = 300;
static int apds_990x_pwave_value = 250;
module_param_named(apds_990x_pwindows_value, apds_990x_pwindows_value, int, S_IRUGO | S_IWUSR | S_IWGRP);
module_param_named(apds_990x_pwave_value, apds_990x_pwave_value, int, S_IRUGO | S_IWUSR | S_IWGRP);

#define APDS990x_DBG(x...) do {\
    if (apds990x_debug_mask) \
        printk(KERN_DEBUG x);\
    } while (0)
...
//	printk("ps_data = %d min_proximity_value = %d pthreshold_h = %d pthresold_l= %d\n",data->psdata,min_proximity_value,pthreshold_h,pthreshold_l);
		...
		APDS990x_DBG("ps_data = %d min_proximity_value = %d pthreshold_h = %d pthresold_l= %d\n",data->ps_data,min_proximity_value,pthreshold_h,pthreshold_l);
...
static void apds990x_als_polling_work_handler(struct work_struct *work)
{
	...
	luxValue = luxValue < 10000 ? luxValue : 10000;
    lux_old = luxValue;
	APDS990x_DBG("%s: lux = %d cdata = %d  irdata = %d pdata = %d \n", __func__, luxValue, cdata, irdata, pdata);
	/* check PS under sunlight */
	...
	if (val == 1) {
		...
		if (data->enable_als_sensor == 0) {
			...
			if (data->enable_ps_sensor) {
				...
				/*	apds990x_set_ppcount(client, 8);*/ /* 2-pulse */
				apds990x_set_ppcount(client, 3); /* 3-pulse */
				   ...
				   const struct i2c_device_id *id)
{
	...
	int err = 0;
       ...
       else if(E_APDS_TYPE_U9700L==apds_type)
       {
        ...
        {  
			/* G710 proximity sensor tuning. */
            apds_990x_pwave_value = 190;
            apds_990x_pwindows_value = 270;
            threshold_value = 200;
        }
		// modify it temporarily, optimize it later	
