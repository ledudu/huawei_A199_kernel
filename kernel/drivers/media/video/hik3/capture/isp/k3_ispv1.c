	...
	camera_sensor *sensor, bool flash_on, camera_scene scene)
{
	...
	if ((false == flash_on) && (sensor->get_ccm_pregain != NULL)) {
		...
		/*
		 *ISP_CMDSET_CCM_PREGAIN_ENABLE(0x1e980)new firmware with new definition
		 * bit[0]	AWB offset enable
		 * bit[1]	AWB pre-gain enable
		 */
		/*do awb offse when capture*/
		SETREG8(ISP_CMDSET_CCM_PREGAIN_ENABLE, 0x01);
	}
	...
	if (true == flash_on) {
		...
		if (0 != isp_hw_data.preview_ae.lum) {
			...
			/*to modify luma change after capture in dark environment*/
	...
	if (sensor->sensor_type == SENSOR_SONY)
			     ...
			     camera_sensor *sensor, bool cold_boot, camera_scene scene)
{
	...
	if (sensor->get_ccm_pregain != NULL) {
		 ...
		 * bit[1]	AWB pre-gain enable
		 */
		/*do awb offset and pre-ccm-gain when preview*/
		SETREG8(ISP_CMDSET_CCM_PREGAIN_ENABLE, 0x03);
	}
			   ...
			   const struct _sensor_reg_t *buf, u32 size, i2c_length length, u8 mask)
{
	...
	/* use AP write mode */
	for (i = 0; i < size; i++) {
		ispv1_write_sensor_byte(index, i2c_addr, buf[i].subaddr, buf[i].value, length, buf[i].mask?buf[i].mask:mask);
	}
#endif
	return 0;
